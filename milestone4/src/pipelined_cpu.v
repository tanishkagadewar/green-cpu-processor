// ============================================================
// Module: pipelined_cpu — 5-Stage Pipelined 16-bit CPU
// Stages: IF → ID → EX → MEM → WB
// Features: Full forwarding, load-use stall, branch flush, FPU
//
// Pipeline registers have explicit enable and flush signals.
// Branch resolution in EX stage with 2-cycle penalty.
// ============================================================
...
module pipelined_cpu (input wire clk, input wire rst);
    ...
    pc_register u_pc (
        .clk(clk), .rst(rst),
        .en(!stall || flush),  // stall holds PC unless flush overrides
    ...
    // EX result mux: FPU or ALU
    assign ex_result = id_ex_is_fpu ? ex_fpu_result : ex_alu_result;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            ex_mem_rs2       <= ex_fwd_b;  // forwarded store data
            ex_mem_rd        <= id_ex_rd;
